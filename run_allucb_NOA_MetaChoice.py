...
def meta_choose_expert_type(x_t, A, f_star, theta_star, expert_types, reward_list_dict, beta_meta, w_t, delta, expert_indices_per_timestep):
    expected_rewards = []
    rewards = {a: f_star(x_t, a, theta_star) for a in A}
    for et in expert_types:
        _, rewards_list = reward_list_dict[et](rewards,x_t)
        if len(rewards_list) == 0:
            expected_rewards.append(-np.inf)  # avoid division by 0
        else:
            expected_rewards.append(np.mean(rewards_list))  # average reward for this expert type

    # Convert to Boltzmann prob:
    exp_vals = np.exp(beta_meta * np.array(expected_rewards))
    probs = exp_vals / np.sum(exp_vals)
    expert_choice = np.random.choice(expert_types, p=probs)
    expert_indices_per_timestep.append(expert_types.index(expert_choice) if w_t >= delta else -1)
    return expert_choice, expert_indices_per_timestep


############################################################################################
############################################################################################

def run_mixucb(data, T, n_actions, n_features, reward_list_dict, expert_types, delta, online_reg_oracle, gamma, eta, beta_meta, mode=None):
    ...
    for i in tqdm(range(T)):
        ...
        if mode in ['mixI', 'mixII', 'mixIII']:
            ...
            # Meta choice: Sample expert
            expert_type, expert_indices_per_timestep = meta_choose_expert_type(context, actions_set, f_star_Linear, theta_star, expert_types, reward_list_dict, beta_meta, width_Ahat, delta, expert_indices_per_timestep)
